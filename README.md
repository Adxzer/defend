...
Defend exposes a small HTTP surface area:
...
The `/guard/input` and `/guard/output` endpoints are designed to slot into any LLM workflow, including:

- Chatbots and assistants
- RAG pipelines
- AI agents and tools
- CI/CD checks for prompts and responses

Full schema details are documented in `ARCHITECTURE.md`.

---

## Who Defend is for

- **Developers** building LLM applications who want a simple way to add AI security guardrails without rewriting their stack.
- **Security engineers** who need a configurable, inspectable guardrail layer for prompt injection defense, PII protection, and scope control.
- **Engineering leaders** who want a clear story for how LLM security is enforced in production.

---

## Configuration and deployment

- **Configuration**: `defend.config.yaml` in the project root.
  - Choose providers (`defend`, `claude`, `openai`).
  - Enable and configure modules (injection, pii, topic, prompt leak, custom).
  - Tune thresholds and guard behavior for different environments.
- **Deployment**:
  - Run as a standalone FastAPI microservice.
  - Package and deploy with Docker.

For detailed configuration reference and environment-specific presets, see `CONFIGURATION.md`.
...
- Improve or add new detection modules.
- Add support for new LLM providers.
- Enhance documentation and examples.

If you plan to contribute, please open an issue first to discuss design or approach. A dedicated `CONTRIBUTING.md` can be added as the project grows.

Planned directions include:

- More built-in AI security policies (e.g. safety-specific checks).
- Deeper CI/CD integration examples.
- Additional providers and deployment templates.
